...
- **Inter-app DNS — short name OR full FQDN, both resolve.** Per the same Learn page: "Container apps can reach each other through any of these methods: FQDN, app name, Dapr service invocation, or custom domain." We default to the full `<app>.internal.<env-default-domain>` form (built from `local.env_default_domain` exported off the env) because it's explicit and grep-able.
