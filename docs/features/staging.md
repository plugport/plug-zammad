...
az containerapp update -n ca-plug-zammad-web -g rg-plug-zammad \
  --image crplugport.azurecr.io/zammad:<sha> \
  --revision-suffix <sha>

# Smoke-test the new revision via its preview FQDN
curl -I https://ca-plug-zammad-web--<sha>.<env>.azurecontainerapps.io/

# Promote to 100%
az containerapp ingress traffic set -n ca-plug-zammad-web -g rg-plug-zammad \
  --revision-weight <sha>=100

# Rollback if needed
az containerapp revision activate -n ca-plug-zammad-web -g rg-plug-zammad --revision <previous>
```
...
- `pg-plug-zammad-staging` (Flexible Server, restored from the latest PITR snapshot of prod)
- `ca-plug-zammad-{web,websocket,worker,scheduler,opensearch,memcached}-staging`
- `cajob-plug-zammad-init-staging`
...
az containerapp job start -n cajob-plug-zammad-init-staging -g rg-plug-zammad

# 3. Smoke tests
./scripts/smoke-test.sh https://ca-plug-zammad-web-staging.<env>.azurecontainerapps.io
...
- Entra SSO against the prod App Registration — staging should use a separate App Registration with a different redirect URI (`https://ca-plug-zammad-web-staging.<env>.azurecontainerapps.io/auth/microsoft_office365_v2/callback`).
